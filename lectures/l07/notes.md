...
Notice what makes this dangerous rather than merely wrong. The contamination is worst at the
*start* of each engine's trajectory, which is where the model is being asked to predict a long
remaining life, and it always drags those early features in the direction of the previous
engine's *end* of life. The bug does not add noise, it adds a bias with the shape of the signal
...
If that pattern sounds familiar, it should. It is the same shape as the ninety-seven ignored
"Power Peg disabled" emails in [L5](../l05/notes.md)'s Knight Capital case: a system emitting a
true signal that no process was obliged to act on. Two failures, two industries, a decade
apart, and the same lesson. An anomaly that is "noted informally" is not monitoring.
...
And when a diagnostic disagrees with your model of the world for months, that is the finding,
not the noise. "Noted but only informally reported" is how this one ended.
:::
...
That is the lesson, and it is a better one than "leakage inflates your score." Your validation
metric is not a leak detector. Whether a leak shows up in it depends on the model you happen to
be using, and the notebook demonstrates this by running the identical leak past four:
...
honest one. A leak is not a mechanism that reliably inflates your number; it is a mechanism that
makes your number meaningless, and it can just as easily flatter you as punish you. The
nearest-neighbours row is the counterweight: change nothing but the model, and the same leak
...
The **scaler statistics diverge by up to 23%** between the two fits, which establishes that the
leak was real before any model is involved. The **`Ridge` gap is 0.002 cycles**, which
establishes that a real leak can be invisible in a metric. The **four-model comparison** shows
the same leak reading as zero, negligible, negative, and clearly positive depending only on the
estimator, which establishes that you cannot audit for this by looking at scores. And two
**silent bugs that were in this notebook until it was checked** are documented in place: a
constant sensor promoted to a "key degradation channel," and a `startswith('sensor2')` filter
that quietly swept in `sensor20` and `sensor21`.
