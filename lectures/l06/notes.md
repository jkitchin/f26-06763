...
Streams usually travel through a **message broker**, a server that sits between the programs that write data and the programs that read it. The writers are called producers: a sensor mote, a plant historian, a web server logging clicks. Each producer sends its records to a named **topic**, such as `temperature`. The readers are called consumers, and each one subscribes to the topics it cares about: a dashboard, an archive job, an alarm. Producers and consumers never talk to each other directly, only to the broker, so adding a fourth consumer changes nothing for the sensors and a slow consumer cannot stall a fast one. [Apache Kafka](https://kafka.apache.org/intro) is the broker most large streaming systems are built on, and it is the one this section describes.
...
:alt: Two motes append records to the two partitions of a temperature topic, each partition a numbered row of cells; a dashboard, an archive and an alarm each point at a different offset.
:width: 100%

A topic with two partitions, each an append-only log numbered by offset. Reading a record does not remove it, so each consumer keeps its own position and a slow archive does not hold up the dashboard. A schematic, drawn by `figures/make_figures.py`.
```

For the lightest-weight sensor case, the pub/sub protocol you will actually meet on devices is [MQTT](https://mqtt.org/), "an extremely lightweight publish/subscribe messaging transport" designed for microcontrollers and lossy networks.
