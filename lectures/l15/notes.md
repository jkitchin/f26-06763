...
[L9](../l09/notes.md) and again in L13; this is the report itself. It is 146
...
millions to tens, and then spend LLM tokens only on the tens. That pipeline is the subject
of L17 in two sessions' time, and this is the half of it you can build
today.
...
Next session takes all of this to the interface. L16 covers the
request and response shape, schema-constrained structured output, validation and repair
loops, prompt caching and cost accounting, and how to evaluate a prompt against a gold set
instead of by reading the output and nodding. The extractor you build there is A8, and the
three habits from today (count the tokens, ground the question, give it a way to say no)
