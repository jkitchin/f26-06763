...
- **Session** L10, Week 5 · **Arc** Machine learning and deep learning
...
- **Assignment** A5, released at L9; this session is its tracking and search half
:::
...
[L9](../l09/notes.md) built the workflow that produces those runs: baselines, cross-validation, metrics, and the discipline of touching the test set once. This session is about keeping the record of that work, and about searching the hyperparameter space without fooling yourself. The two are the same problem seen twice. A hyperparameter search is a machine for generating hundreds of runs and picking the best one, and picking the best one is exactly where an unrecorded, unexamined study quietly lies to you. So we log every run with enough detail to rebuild it, we search with a strategy rather than by hand, and we read the results knowing that the best validation score is an optimistic number by construction.

The dataset stays the same as L9: the UCI Combined Cycle Power Plant set, 9,568 hourly records of four ambient measurements (temperature, exhaust vacuum, pressure, humidity) predicting net electrical output in megawatts. It is a steady-state surrogate problem, the kind that recurs for pump curves and engine maps, and it is small and clean enough that a full search runs in class.
...
That list is specific, and it ties this session back to the rest of the course. Log the **hyperparameters** and the **metrics** (per fold, not just the average, so you can see the variance). Log the **git commit SHA** of the code, so the exact program is recoverable. Log the **dataset version or content hash** from [L8](../l08/notes.md), so the exact inputs are pinned. Log the **random seed** and the **environment**, which is the `uv.lock` from [L2](../l02/notes.md). And log the **artifacts**: the fitted pipeline, the plots, the feature importances. The rule from L2 returns with more force here, because a search multiplies the number of runs by a hundred: one run equals one fact you can reproduce, and a run you cannot rebuild is a number you cannot defend.
...
Nine trials each. If only the horizontal hyperparameter matters, grid search tries three distinct values of it and random search tries nine, because the grid spends the other six trials repeating those three values against an axis that does not change the score. Redrawn after Bergstra and Bengio (2012). Generated by `figures/make_figures.py`.
```
...
Best validation RMSE so far, over 30 trials of a gradient-boosting model on CCPP. TPE reaches a good configuration in fewer trials than random sampling, but both finish close: 3.326 MW for TPE against 3.335 MW for random. On an easy problem the sampler matters less than the fact that you searched at all. Generated by `figures/make_figures.py`.
```
...
The power-plant result is the caution: TPE beat random by 0.009 MW, a difference smaller than the fold noise. Bayesian optimization earns its complexity on expensive objectives with many interacting hyperparameters, such as a deep network that takes hours to train. For a fast model on a clean tabular problem, random search with a sensible budget is usually enough, and a better feature or a cleaner label from [L7](../l07/notes.md) and [L8](../l08/notes.md) will move the score more than any sampler.
...
- [UCI Combined Cycle Power Plant dataset](https://archive.ics.uci.edu/dataset/294/combined+cycle+power+plant). The 9,568-record set (Tüfekci 2014) used here and in L9.

## Assignment

A5, "Model-selection study with tracked experiments," was released at [L9](../l09/notes.md) and is due about a week later. This session is its second half: take one engineering regression dataset from data to a defended model choice, with every run tracked in MLflow, a systematic hyperparameter search, and a single honest test estimate reported at the end. The full spec and rubric are in [A5](../../course/assignments/a05.md); this paragraph is a pointer, not the rubric.
