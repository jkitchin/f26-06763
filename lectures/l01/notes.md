...
semester on. Everything else is this course. Generated by `figures/make_figures.py`.
```
...
The rest of this session is a survey. Each subsection below is a layer of the system
diagram, and each is a later arc of the course. The goal today is not competence in any of
them, it is a mental map: knowing what each layer is for, what breaks in it, and where in
the semester we come back to it. Read this section for orientation rather than detail.
...
- [OWASP Top 10 for LLM Applications](https://owasp.org/www-project-top-10-for-large-language-model-applications/). The standard catalogue of LLM-specific security risks, prompt injection first among them.
