...
## The data
The fault-free training file from [Rieth et al. (2017)](https://doi.org/10.7910/DVN/6C3JR1), the same simulations Lecture 8 uses. Download it once (25 MB) into `data/`:

```bash
mkdir -p data
curl -L -o data/TEP_FaultFree_Training.RData \
  https://dataverse.harvard.edu/api/access/datafile/3031241
```

Read it with `pyreadr` (`pyreadr.read_r(path)["fault_free_training"]`), then convert to Polars. Cast `simulationRun` and `sample` to integers, and save a Parquet copy so you only parse the R file once. **Do not commit the data.**
...
| Data | `data/*.RData` or a Parquet copy, not committed |
