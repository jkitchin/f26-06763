...
It writes `evidence.pdf`. Upload that. Nothing to install: the script imports nothing outside the standard library, so `uv` here is only supplying an interpreter. `python3 a02-evidence.py ...` does the same thing if you would rather.

**Use `--no-project`.** A plain `uv run` syncs your project before running anything, so a stale `uv.lock` or one unresolvable dependency would leave you with no report at all — one unrelated mistake taking down a submission that is otherwise fine. `--no-project` skips that entirely, which is safe precisely because this script needs nothing from your environment.
