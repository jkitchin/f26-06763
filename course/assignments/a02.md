...
### The join, and what an inner join cannot see

Query (b) asks which sensors dropped the most readings, and there is a modelling
decision hiding in it that the SQL syntax will not prompt you for. A query built
from the readings table alone can only rank sensors that produced readings. Think
about which table your query should start `FROM`, and what
[`LEFT JOIN`](https://www.sqlite.org/lang_select.html) does that a plain `JOIN`
does not — you have a roster of every mote that was deployed, and it is not the
same list as the motes that reported. Whatever you decide, say in your report
what you counted and why; there are several defensible answers and the reasoning
is what earns the points.

[Common table expressions](https://www.sqlite.org/lang_with.html) (`WITH … AS`)
make this readable: compute the gaps in one, aggregate per sensor in the next,
