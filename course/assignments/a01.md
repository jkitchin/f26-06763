...
It writes `evidence.html`. Open it in a browser, print to PDF, and upload the PDF. Run it with the system `python3` rather than `uv run`, because the first thing it does is delete `.venv` and rebuild it, which is the evidence for the first 30 points.
...
The report is generated, so add `a01-evidence.py`, `evidence.html`, and `evidence.pdf` to your `.gitignore` rather than committing them.
