...
## Goal
Take the broken demo notebook from Lecture 1 and land it in a `uv`-managed, git-tracked project on your own machine that someone else could rebuild and rerun with two commands. You are not writing a new analysis. The analysis already exists, it just cannot be reproduced, and your job is to fix that and give it a home. This scaffold is the template you reuse all semester.

## Learning outcomes assessed
- Create a deterministic environment with `uv`, meaning a pinned interpreter and a committed lockfile.
- Structure a project with a `src/` layout and a `.gitignore` that keeps raw data and the virtual environment out of git.
...
## What you hand in
**One PDF, uploaded to Canvas.** Everything else stays in your working directory. You build a git repository and commit to it, but it is a local repository: nothing is pushed anywhere, there is no remote, and you do not need a GitHub account for this assignment. The PDF is generated from your project by a script we provide, which runs the commands below and records what they actually printed, then embeds your `.gitignore`, your `pyproject.toml`, everything in `src/`, your notebook's code cells, and your README. Grading reads that one document.
...
The report is generated, so add `a01-evidence.py` and `evidence.pdf` to your `.gitignore` rather than committing them. The script does not count its own files against you, nor `.DS_Store` and its Windows equivalents, but anything else left uncommitted fails the clean-tree check and the report names the file.

## Definition of done
The script decides each of these from your project. Run it early, not five minutes before the deadline.
...
| Criterion | Pts |
|---|---|
| Transcript shows the environment rebuilding from the lockfile, with `uv.lock` and `.python-version` tracked | 30 |
| Repo hygiene: `src/` layout, `.gitignore` doing its job, no data file tracked, clean tree | 20 |
| The embedded source shows importable functions and a working `--seed` entry point | 20 |
| The three Lecture 1 defects fixed, and the notebook's cells numbered in order | 15 |
| MLflow table shows runs for two different seeds, with a parameter and a metric | 10 |
| README, embedded in the report: rebuild and reproduce commands that match the transcript | 5 |
...
- Add a git pre-commit hook that runs your test, so a commit that breaks it is harder to make.
